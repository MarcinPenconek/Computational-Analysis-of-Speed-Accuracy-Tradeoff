...
#  }
  ...
  legend(650, 40, legend = "c = 51.2%", cex = opis, bty = "n")

}


#-----------------------------------------------
# FIGURE 2c: Phase space
#-----------------------------------------------
...
#----------------------------
# FIGURE 2a: ATTRACTOR SETS
#----------------------------

dane1 <- read.csv2("~/Marcin/Studia/NeuroNetwork Model II/SAT paper/Method/Dist_FR_A.csv", row.names=1)
dane2 <- read.csv2("~/Marcin/Studia/NeuroNetwork Model II/SAT paper/Method/Dist_FR_B.csv", row.names=1)
dane3 <- read.csv2("~/Marcin/Studia/NeuroNetwork Model II/SAT paper/Method/Dist_FR_Spontaneous.csv", row.names=1)
...
#------------------------------------------------
# FIGURE 4ab: Alternative Hypotheses
#------------------------------------------------
...
Firing_rate_plot <- function(x, y1, y2, y3, dec_1, dec_2) {
  ...
  plot(x, y1, ylim = c(0,70), xlim = c(-event_starts,1000), type = "l",  
       xlab = "Time (ms)", ylab = "Firing Rate (Hz)", 
       #xlab = "", ylab = "", 
       cex.main = 1.5, cex.axis = osie, cex.lab = opis, col = "red", lwd = grubosc)
...
#-------------------------------------
# Threshold Tuning - Illustration (4b)
...
for(i in 1:1) {
  ...
  Firing_rate_plot(firing_rate[ ,1] - event_starts, firing_rate[ ,2], firing_rate[ ,3], firing_rate[ ,4], 0, 0)
  
  abline(h = 50, col = "light grey", lwd=grubosc)
  abline(h = 45, col = "grey", lwd=grubosc)
  
  legend(-500, 70, legend = c("Threshold 50Hz", "Threshold 45Hz"), cex = opis, lwd=c(3,3), col=c("light grey","grey"), bty = "n")
...
#-------------------------------------
# Gain Modulation - Illustration (4a)
...
for(i in 1:1) {
  ...
  lines(firing_rate[, 1] - event_starts, firing_rate[ ,2], type = "l", col = "red", lwd=grubosc)
  lines(firing_rate[, 1] - event_starts, firing_rate[ ,3], type = "l", col = "blue", lwd=grubosc)
  
  abline(h = 50, col = "grey", lwd=grubosc)
  
  legend(-500, 70, legend = c("Low Excitability", "High Excitability"), cex = opis, lwd=c(3,3), col=c("pink","red"), bty = "n")
